...
def interpolate(raw_data: list, x_values: str, interpolation_points: list, title: str):
    ...
    graph(raw_data, title=title, interpolated_data=interpolated_data)                # Render original + interpolated data


# Render graphs:
def graph(data_y, title='Data', interpolated_data=None):
    plt.plot(data_y, '-go', label='Actual Data(NDVI)', alpha=0.3)

    if interpolated_data:
        plt.plot(interpolated_data, ':r', label='Interpolated Data', alpha=0.7)
